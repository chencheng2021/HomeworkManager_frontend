...
export function validate_student_imported_data(item) {
    ...
    if (typeof item.studentNo !== 'number'){
    ...
    }else if (item.socialAccount === 'undefined'){
        Message.error('表格数据必须包含 [社交账号] 属性的列')
    }else if (item.fatherName === 'undefined'){
        Message.error('表格数据必须包含 [父亲] 属性的列')
    }else if (item.motherName === 'undefined'){
        Message.error('表格数据必须包含 [母亲] 属性的列')
    }else if (item.fatherContact === 'undefined'){
        Message.error('表格数据必须包含 [父亲手机] 属性的列')
    }else if (item.motherContact === 'undefined'){
